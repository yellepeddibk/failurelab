        ...
        metrics.extend(
            [
                ...
                MetricResult(
                    ...
                    "USD",
                    "lower_is_better",
                    "no successful traces with known cost",
                ),
            ]
        )
...
def _extend_agent_metrics(
    ...
    metrics.extend(
        [
            ...
            MetricResult(
                "agent_average_steps",
                mean(step_counts) if step_counts else None,
                "Average steps for traces with agent steps",
                len(step_counts),
                len(traces) - len(step_counts),
                sum(step_counts) if step_counts else None,
                len(step_counts),
                "count",
                "lower_is_better",
                None if step_counts else "no traces with agent steps",
            ),
            MetricResult(
                "agent_max_steps",
                max(step_counts) if step_counts else None,
                "Maximum agent steps",
                len(step_counts),
                len(traces) - len(step_counts),
                None,
            ...
            MetricResult(
                ...
                len(known_tool),
                len(tool_steps) - len(known_tool),
                len(success_tool) if known_tool else None,
                len(known_tool),
                "ratio",
                "higher_is_better",
